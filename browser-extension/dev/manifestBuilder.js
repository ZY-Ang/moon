/*
 * Copyright (c) 2018 moon
 */

import logo from "../../assets/icons/logo_128.png";
import {description, homepage, version} from "../package.json";

const manifest = {
    manifest_version: 2,

    name: "Moon",
    short_name: "Moon",
    description: description,
    version: version,
    homepage_url: homepage,

    // Synchronize logo name with other app scripts
    icons: {
        16: logo,
        32: logo,
        128: logo
    },

    web_accessible_resources: [
        "app.js", // Required for re-injection by Browser.Runtime
        "static/*",
        "app.css"
    ],

    permissions: [
        "identity",
        "storage",
        "unlimitedStorage",
        // TODO: Implement least-privilege for REST requests
        "http://*/*",
        "https://*/*",
        "activeTab",
        "tabs"
    ],

    // content_security_policy: "script-src 'self' 'unsafe-eval'; object-src 'self'",
    browser_action: {
        default_title: "Moon"
    },

    background: {
        scripts: ["background.js"],
        persistent: false
        // Only persistent pages are supported?
    },

    content_scripts: [
        {
            matches: ["<all_urls>"],
            js: ["app.js"]
        }
    ]
 };

const generateManifest = () => {
    if (process.env.BROWSER === "chrome") {
        manifest.key = "";
    } else if (process.env.BROWSER === "firefox") {
        manifest.browser_specific_settings = {
            "gecko":
                {
                    "id": "ext@paywithmoon.com",
                    //If your extension is not hosted on AMO, you must specify the location of your update manifest in your extension.
                    "update_url": "https://paywithmoon.com/updates.json"
                }
        }
    }
    return manifest;
}

// Note: You will need to destroy the compiled version of this module in the build process.
module.exports = generateManifest();

